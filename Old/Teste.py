...
with pd.ExcelWriter(NOME_ARQUIVO_EXCEL, engine="xlsxwriter", date_format="dd/mm/yyyy") as writer:
    ...
    number_format = workbook.add_format({'num_format': '#,##0'}) #*****CORRIGIR FORMATO
        ...
        for col_num, value in enumerate(row_data):
            if pd.isna(value):
                ...
                if column_name == "Número NF":
                    ...
                    if pd.notna(value):
                        worksheet.write_string(row_num + 1, col_num, f"{value:014}", cnpj_format)
                    else:
                        worksheet.write(row_num + 1, col_num, "", cell_format)
                elif column_name == "CNPJ/CPF Tomador":
                    if pd.notna(value):
                        str_value = f"{int(value):014}" if isinstance(value, (int, float)) else str(value).zfill(14)
                        if len(str_value) == 14:
                            worksheet.write_string(row_num + 1, col_num, str_value, cnpj_format)
                        elif len(str_value) == 11:
                            worksheet.write_string(row_num + 1, col_num, str_value, cpf_format)
                        else:
                            worksheet.write_string(row_num + 1, col_num, str_value, cell_format)
                    else:
                        worksheet.write(row_num + 1, col_num, "", cell_format)
                else:
